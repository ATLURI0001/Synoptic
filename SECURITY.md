...
Synoptic is a local-first client for **public** data. It is built for exploration, demos, and learning — not as a hardened production service. This document explains the security model so you can run it safely and report issues responsibly.

## Reporting a vulnerability

Please report security issues **privately** — do not open a public issue for anything exploitable.

- Use GitHub's [private vulnerability reporting](https://github.com/ATLURI0001/synoptic/security/advisories/new) (Security tab → "Report a vulnerability"), or
...
1. **Google Maps API key** — loads Photorealistic 3D Tiles directly and powers Synoptic place search. **Restrict it** (HTTP referrer + API restriction to the required Google APIs) in the Google Cloud Console. An unrestricted key in a public deployment can be abused and billed to you.
...
the optional server key manually in the environment file. Pinokio uses its
ignored `pinokio/ENVIRONMENT` instead, with app values and blanks taking
...
- **Pinokio LAN and Cloudflare sharing are refused.** The current supported
  ...
  sharing value is therefore discarded rather than honored, and Synoptic starts
  on loopback only. Use a separately reviewed authentication proxy for remote
